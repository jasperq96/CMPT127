...
void ship_draw( const ship_t* ship )
{
  ...
  */ 
  for(int n = -1; n <= 1; ++n){
    for(int m = -1; m <= 1; ++m){
      draw_triangle(ship->x + n + dx, ship->y + m + dy, ship->x + n + lx, ship->y + m + ly, ship->x + n + rx, ship->y + m + ry, shipcolor);
      if (ship->thrust){
        draw_triangle(ship->x + n - dx, ship->y + m - dy, ship->x + n + lx, ship->y + m + ly, ship->x + n + rx, ship->y + m + ry, thrustcolor);
      }
    }
  }
}
...
void roid_draw( const roid_t* roid )
{
  ...
  */
  for(int n = -1; n <= 1; ++n){
    for(int m = -1; m <= 1; ++m){
      draw_rectangle( (roid->x-roid->width/2.0) + n, (roid->y-roid->height/2.0) + m, (roid->x+roid->width/2.0) + n, (roid->y+roid->height/2) + m, roid->color );
    }
  }
}
...
int shot_roid_hit( const shot_t* shot, const roid_t* roid )
{
  ...
  for(int n = -1; n <= 1; ++n){
    for(int m = -1; m <= 1; ++m){
      if( (shot->x + n >= roid->x - roid->width/2) && (shot->x + n <= roid->x + roid->width/2) && (shot->y + m >= roid->y - roid->height/2) && (shot->y + m <= roid->y + roid->height/2) ){
        return 1;
      }
    }
  }
